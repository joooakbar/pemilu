    ...
    if (!sanityKandidat || sanityKandidat.length === 0) {
      ...
      for (const k of sanityKandidat) {
        if (!k._id || !k.nomorUrut || !k.namaPaslon) {
          kandidatResult.errors.push(`Lewati "${k.namaPaslon || k._id}" — noUrut atau nama kosong`)
          continue
        }
        try {
          const existing = await prisma.kandidat.findFirst({ where: { sanityId: k._id } })

          if (existing) {
            if (existing.nama !== k.namaPaslon || existing.noUrut !== k.nomorUrut) {
              const conflict = await prisma.kandidat.findFirst({
                where: { noUrut: k.nomorUrut, NOT: { sanityId: k._id } },
              })
              if (conflict) {
                kandidatResult.errors.push(`"${k.namaPaslon}": noUrut ${k.nomorUrut} sudah dipakai`)
                continue
              }
            ...
            if (electionResult.electionId) {
              await prisma.kandidat.create({
                data: { noUrut: k.nomorUrut, nama: k.namaPaslon, sanityId: k._id },
              })
