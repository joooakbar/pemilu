  ...
  return (
    <div className="rounded-xl border bg-card overflow-hidden">
      ...
      <div className="border-t px-5 py-3 bg-secondary/20 text-xs text-muted-foreground space-y-1">
        ...
        <p>
          ③ Status election <strong>TIDAK</strong> berubah saat sync (hanya ubah
          via Emergency Stop)
        </p>
      </div>
    </div>
...
function SyncSection({
  ...
  return (
    <div
      className={`rounded-lg border p-4 space-y-2 ${hasError ? "border-amber-200 bg-amber-50" : "border-green-200 bg-green-50"}`}
    >
      <div
        className={`flex items-center gap-2 font-medium text-sm ${hasError ? "text-amber-800" : "text-green-800"}`}
      >
        {icon} {title}
      </div>
