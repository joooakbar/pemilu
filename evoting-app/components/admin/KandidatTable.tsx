    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-sm text-muted-foreground">
          <span className="font-medium">{sanityData.length}</span> kandidat di
          Sanity &nbsp;·&nbsp;
          <span className="font-medium">{dbList.length}</span> terdaftar di
          database
        </p>
      ...
      {result && (
        <div
          className={`rounded-lg border p-4 space-y-2 ${result.errors.length > 0 ? "bg-amber-50 border-amber-200" : "bg-green-50 border-green-200"}`}
        >
          <div
            className={`flex items-center gap-2 text-sm font-semibold ${result.errors.length > 0 ? "text-amber-700" : "text-green-700"}`}
          >
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {sanityData.map((k) => {
                ...
                const synced = !!db;
                const hasChanges =
                  db && (db.nama !== k.namaPaslon || db.noUrut !== k.nomorUrut);
                return (
                  <tr
                    key={k._id}
                    className="hover:bg-secondary/20 transition-colors"
                  >
                    <td className="px-4 py-3 font-black text-xl text-primary">
                      {k.nomorUrut}
                    </td>
                    <td className="px-4 py-3">
                      {k.foto?.asset?.url ? (
                        <Image
                          src={k.foto.asset.url}
                          alt={k.namaPaslon}
                          width={44}
                          height={44}
                        ...
                        <div className="w-11 h-11 rounded-full bg-secondary flex-center text-xl">
                          👤
                        </div>
                      )}
                    </td>
      ...
      <div className="rounded-lg bg-secondary/40 border p-4 text-xs text-muted-foreground space-y-1.5">
        ...
        <p>
          ① Tambah/edit kandidat di <strong>Sanity Studio</strong> (foto, visi,
          misi, video)
        </p>
        <p>
          ② Klik <strong>Publish</strong> di Studio — konten langsung tampil di
          portal voter
        </p>
        <p>
          ③ Kembali ke halaman ini → klik{" "}
          <strong>Sinkronisasi ke Database</strong>
        </p>
        <p>
          ④ Data nomorUrut & nama masuk ke tabel{" "}
          <code className="bg-secondary px-1 rounded">kandidat_ref</code> untuk
          keperluan hitung suara
        </p>
        <p className="text-amber-600 font-medium">
          ⚠️ Jangan hapus kandidat dari Sanity jika sudah ada suara masuk di
          database!
        </p>
      </div>
    </div>
