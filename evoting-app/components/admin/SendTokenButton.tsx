...
export default function SendTokenButton({ electionId, electionStatus }: Props) {
  ...
  const run = async () => {
    ...
    try {
      ...
      setResult(j2.data);
      toast.success(
        `Token berhasil dikirim ke ${j2.data.waSent + j2.data.emailSent} pemilih`,
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-muted-foreground">
              Generate & broadcast token OTP via WhatsApp / Email
            </p>
          </div>
        </div>
        {open ? (
        ...
        <div className="border-t px-5 py-4 space-y-5">
          {isDisabled && (
            <div className="rounded-lg bg-amber-50 border border-amber-200 p-3 text-sm text-amber-700">
              ...
              <strong>ACTIVE</strong> untuk mengirim token.
            </div>
          )}
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            <input
              ...
              onChange={(e) =>
                setOpts((o) => ({ ...o, generate: e.target.checked }))
              }
              className="mt-0.5 w-4 h-4 accent-primary"
            />
            <div>
              <p className="text-sm font-medium">
                Generate token baru terlebih dahulu
              </p>
              <p className="text-xs text-muted-foreground">
                Hapus token lama & buat token baru untuk semua pemilih yang
                belum memilih
              </p>
            </div>
          </label>
              ...
              {[
                {
                  ...
                  desc: "Pemilih yang punya nomor HP",
                },
                {
                  val: "email",
                  label: "📧 Email",
                  desc: "Pemilih yang punya alamat email",
                },
              ].map(({ val, label, desc }) => (
                <label
                  key={val}
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              onChange={(e) =>
                setOpts((o) => ({ ...o, expiredJam: +e.target.value }))
              }
