        ...
        <div className="rounded-xl border bg-card p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
            {[
              {
                role: "ADMIN",
                desc: "Akses penuh: semua fitur termasuk emergency stop & berita acara",
              },
              {
                role: "PANITIA",
                desc: "Akses: dashboard, DPT, kirim token. Tidak bisa hapus kandidat atau emergency stop",
              },
              {
                role: "SAKSI",
                desc: "Akses hanya lihat: dashboard & statistik real-time",
              },
            ].map(({ role, desc }) => (
              <div
                key={role}
                className={`rounded-lg border p-2.5 ${form.role === role ? "border-primary/50 bg-primary/5" : ""}`}
              >
                <span
                  className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-bold border mb-1 ${ROLE_COLOR[role as Role]}`}
                >
                  {role}
                </span>
          ...
          <tbody className="divide-y">
            {loading
              ...
              : filtered.map((u) => (
                  <tr
                    key={u.id}
                    className={`hover:bg-secondary/20 transition-colors ${!u.isActive ? "opacity-50" : ""}`}
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-xs px-2 py-0.5 rounded border font-medium ${ROLE_COLOR[u.role]}`}
                      >
                        {ROLE_LABELS[u.role]}
                      </span>
                    </td>
