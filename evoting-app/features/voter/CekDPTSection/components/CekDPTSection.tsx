...
import { CekDPTProps } from "../types/cekdpt.types";

export default function CekDPTSection({ idPemilihan }: CekDPTProps) {
  const { nik, loading, status, result, handleNIKChange, handleCekDPT } =
    useCekDPT(idPemilihan);
  return (
    <section className="section cek-dpt-section" id="cek-dpt">
      <div className="section-header">
        <div className="section-eyebrow">Daftar Pemilih Tetap</div>
        <h2>Cek Status DPT Anda</h2>
        <p>Masukkan NIK KTP Anda untuk memastikan bahwa nama Anda terdaftar!</p>
      </div>
        ...
        {status !== "idle" && (
          ...
          >
            <div className="cek-result-card">
              <div className="cek-icon">{status === "found" ? "✅" : "❌"}</div>
                ...
                {status === "found" && result && (
                  <div className="cek-detail-row">
                    ...
                    <div className="cek-detail-item">
                      ...
                      <span>
                        {result.hasVoted ? "Sudah Memilih" : "Belum Memilih"}
                      </span>
                    </div>
                  </div>
                )}
