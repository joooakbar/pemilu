...
export default function SendTokenButton({
  ...
  const [result, setResult] =
    useState<SendTokenResultData | null>(null)

  const [opts, setOpts] = useState(
    DEFAULT_SEND_TOKEN_OPTIONS
  )

  const { loading, run } = useSendToken(
    electionId,
    opts
  )

  const isDisabled =
    !['ACTIVE', 'DRAFT'].includes(electionStatus)
